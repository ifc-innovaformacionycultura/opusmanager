...
import { FichajeConfigPanel, FichajePlantillaEventoButton } from "../components/FichajeConfigPanel";
...
const ProgramaMusicalBackend = ({ event, legacyProgram, isSuperAdmin, currentUserId }) => {
  ...
  // Iter F4.2 — estado de save por fila: 'idle' | 'pending' | 'saving' | 'saved' | 'error'
  const [saveState, setSaveState] = useState({});
  // Iter F4.2 — estado del typeahead por fila: { rowId: { results, focused, loading } }
  ...
  // Iter F4.2 — Patch debounced quirúrgico (no recarga toda la tabla):
  // solo actualiza los campos devueltos por el server en la fila concreta.
  const patchTimers = React.useRef({});
  const flushRow = React.useCallback(async (eo_id) => {
    const t = patchTimers.current[eo_id];
    if (!t) return;
    clearTimeout(t.timer);
    delete patchTimers.current[eo_id];
    setSaveState(s => ({ ...s, [eo_id]: 'saving' }));
    try {
      const res = await api.patch(`/api/gestor/archivo/evento/${eventoId}/obras/${eo_id}`, t.payload);
      const updated = res.data?.evento_obra;
      if (updated) {
        // Merge quirúrgico: respeta los campos que el usuario pueda estar editando ahora
        setRows(prev => prev.map(r => r.id === eo_id ? {
          ...r,
          obra_id: updated.obra_id ?? r.obra_id,
          estado: updated.estado ?? r.estado,
          obra: updated.obra ?? r.obra,
          // Si server hizo match catálogo y devolvió titulo_provisional=null, reflejarlo:
          titulo_provisional: updated.titulo_provisional !== undefined ? updated.titulo_provisional : r.titulo_provisional,
        } : r));
      }
      setSaveState(s => ({ ...s, [eo_id]: 'saved' }));
      setTimeout(() => setSaveState(s => {
        if (s[eo_id] !== 'saved') return s;
        const n = { ...s }; delete n[eo_id]; return n;
      }), 1500);
    } catch (e) {
      setSaveState(s => ({ ...s, [eo_id]: 'error' }));
  ...
  const updateField = (eo_id, field, value) => {
    ...
    setSaveState(s => ({ ...s, [eo_id]: 'pending' }));
    const prev = patchTimers.current[eo_id] || { payload: {}, timer: null };
    if (prev.timer) clearTimeout(prev.timer);
    const newPayload = { ...prev.payload, [field]: value === '' ? null : value };
    const timer = setTimeout(() => flushRow(eo_id), 800);
    patchTimers.current[eo_id] = { payload: newPayload, timer };
  };

  // Iter F4.2 — Botón "Guardar todo": fuerza flush inmediato de filas pendientes.
  const guardarTodo = async () => {
    const ids = Object.keys(patchTimers.current);
    if (ids.length === 0) {
  ...
  // Iter F4.2 — Typeahead: busca obras del catálogo y muestra dropdown.
  const searchTimers = React.useRef({});
  const onTituloChange = (eo_id, value) => {
    updateField(eo_id, 'titulo_provisional', value);
    setSearch(s => ({ ...s, [eo_id]: { ...(s[eo_id] || {}), focused: true, loading: value.length >= 2 } }));
    if (searchTimers.current[eo_id]) clearTimeout(searchTimers.current[eo_id]);
    if (value.length < 2) {
      setSearch(s => ({ ...s, [eo_id]: { ...(s[eo_id] || {}), results: [], loading: false } }));
      return;
    }
    searchTimers.current[eo_id] = setTimeout(async () => {
      try {
        const r = await api.get(`/api/gestor/archivo/obras?q=${encodeURIComponent(value)}`);
        const obras = (r.data?.obras || r.data || []).slice(0, 8);
        setSearch(s => ({ ...s, [eo_id]: { ...(s[eo_id] || {}), results: obras, loading: false, focused: true } }));
      } catch {
        setSearch(s => ({ ...s, [eo_id]: { ...(s[eo_id] || {}), results: [], loading: false } }));
      }
    }, 200);
  ...
  const seleccionarObra = async (eo_id, obra) => {
    ...
    if (patchTimers.current[eo_id]) { clearTimeout(patchTimers.current[eo_id].timer); delete patchTimers.current[eo_id]; }
    if (searchTimers.current[eo_id]) clearTimeout(searchTimers.current[eo_id]);
    setSearch(s => ({ ...s, [eo_id]: { ...(s[eo_id] || {}), focused: false, results: [] } }));
    setSaveState(s => ({ ...s, [eo_id]: 'saving' }));
    try {
      const res = await api.patch(`/api/gestor/archivo/evento/${eventoId}/obras/${eo_id}`, {
        obra_id: obra.id,
        estado: 'confirmada',
        titulo_provisional: null,
      });
      const updated = res.data?.evento_obra;
      setRows(prev => prev.map(r => r.id === eo_id ? {
        ...r,
        ...
        estado: 'confirmada',
        titulo_provisional: null,
        obra: updated?.obra || { id: obra.id, titulo: obra.titulo, autor: obra.autor },
      } : r));
      setSaveState(s => ({ ...s, [eo_id]: 'saved' }));
      setTimeout(() => setSaveState(s => {
        if (s[eo_id] !== 'saved') return s;
      ...
      setSaveState(s => ({ ...s, [eo_id]: 'error' }));
      alert('No se pudo vincular: ' + (e.response?.data?.detail || e.message));
    }
  };
    ...
    <div className="overflow-x-auto" data-testid="programa-musical-backend">
      ...
      <div className="mb-3 flex items-center gap-2 flex-wrap">
        ...
        <button
          ...
          data-testid="btn-guardar-todo-programa"
            ...
            return (
              <tr key={r.id} className="border-b border-slate-100" data-testid={`programa-row-${r.id}`}>
                ...
                <td className="px-3 py-2 relative">
                  {isConfirmada ? (
                    <div className="px-2 py-1 text-sm text-slate-700 bg-emerald-50 rounded flex items-center gap-2">
                      ...
                      <button
                        type="button"
                        onClick={() => updateField(r.id, 'obra_id', null)}
                        className="ml-auto text-[10px] text-slate-500 hover:text-red-600"
                        data-testid={`btn-desvincular-obra-${r.id}`}
                    ...
                    <>
                      <input
                        ...
                        onChange={(e) => onTituloChange(r.id, e.target.value)}
                        onFocus={() => setSearch(s => ({ ...s, [r.id]: { ...(s[r.id] || {}), focused: true } }))}
                        onBlur={() => setTimeout(() => setSearch(s => ({ ...s, [r.id]: { ...(s[r.id] || {}), focused: false } })), 200)}
                        className="w-full px-2 py-1 border border-slate-200 rounded text-sm"
                        placeholder="Escribe para buscar en el catálogo…"
                        data-testid={`input-titulo-${r.id}`}
                      />
                      {search[r.id]?.focused && (search[r.id]?.results || []).length > 0 && (
                        <ul className="absolute z-30 left-3 right-3 mt-1 bg-white border border-slate-200 rounded-lg shadow-lg max-h-56 overflow-y-auto"
                            data-testid={`typeahead-obras-${r.id}`}>
                          {(search[r.id]?.results || []).map(obra => (
                            <li key={obra.id}>
                              <button
                                type="button"
                                onMouseDown={(e) => e.preventDefault()}
                                onClick={() => seleccionarObra(r.id, obra)}
                                className="w-full text-left px-3 py-1.5 hover:bg-emerald-50 text-xs"
                                data-testid={`typeahead-obra-${r.id}-${obra.id}`}
                              >
                                <div className="font-medium text-slate-800 truncate">{obra.titulo}</div>
                                <div className="text-[10px] text-slate-500 truncate">{obra.autor}{obra.codigo ? ` · ${obra.codigo}` : ''}</div>
                              </button>
                            </li>
                          ))}
                        </ul>
                      )}
                      {search[r.id]?.focused && search[r.id]?.loading && (
                        <div className="absolute z-30 left-3 right-3 mt-1 bg-white border border-slate-200 rounded-lg shadow-lg px-3 py-1.5 text-[10px] text-slate-500"
                             data-testid={`typeahead-loading-${r.id}`}>
                ...
                <td className="px-3 py-2 text-xs">
                  ...
                  {/* Indicador estado de guardado por fila */}
                  {saveState[r.id] === 'pending' && (
                    <span className="ml-1 text-[10px] text-amber-600" data-testid={`save-state-${r.id}`}>● Pendiente</span>
                  )}
                  {saveState[r.id] === 'saving' && (
                    <span className="ml-1 text-[10px] text-blue-600" data-testid={`save-state-${r.id}`}>● Guardando…</span>
                  )}
                  {saveState[r.id] === 'saved' && (
                    <span className="ml-1 text-[10px] text-emerald-600" data-testid={`save-state-${r.id}`}>✓ Guardado</span>
                  )}
                  {saveState[r.id] === 'error' && (
                    <span className="ml-1 text-[10px] text-red-600" data-testid={`save-state-${r.id}`}>⚠ Error</span>
                  )}
                </td>
                <td className="px-3 py-2">
                  <button
...
// ============================================================
// Iter F4 — FichajeConfigPanel y FichajePlantillaEventoButton extraídos a
// /app/frontend/src/components/FichajeConfigPanel.js (refactor F4.2 · 2026-05-03)
// ============================================================
